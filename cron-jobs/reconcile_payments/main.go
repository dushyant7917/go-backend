...
func main() {
	...
	log.Printf("[%s] Starting reconcile payments job (Cron D)\n", timestamp)
	...
	// Cron D: Reconcile payments - sync state from Razorpay for missed webhook events
	// Fetches payment/order status from Razorpay and updates local records
	if err := recurringPaymentService.ReconcilePayments(); err != nil {
		log.Printf("[%s] ✗ Reconcile payments job failed: %v\n", timestamp, err)
	} else {
		log.Printf("[%s] ✓ Reconcile payments job completed successfully\n", timestamp)
	}
}
