...
func main() {
	...
	log.Printf("[%s] Starting charge pending payments job (Cron C)\n", timestamp)
	...
	// Cron C: Charge pending payments - create recurring payment via Razorpay SDK
	// Finds payment attempts in created/pending state created 25+ hours ago and charges them
	if err := recurringPaymentService.ChargePendingPayments(); err != nil {
		log.Printf("[%s] ✗ Charge pending payments job failed: %v\n", timestamp, err)
	} else {
		log.Printf("[%s] ✓ Charge pending payments job completed successfully\n", timestamp)
	}
}
