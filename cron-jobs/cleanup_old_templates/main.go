...
func main() {
	...
	if len(os.Args) < 2 {
		log.Fatal("Error: days argument is required\nUsage: go run cleanup_old_templates.go <days>\nExample: go run cleanup_old_templates.go 7")
	}
	...
	log.Printf("[%s] Starting cleanup of templates older than %d days\n", timestamp, days)
	...
	log.Printf("[%s] Cutoff date: %s (templates created before this will be deleted)\n", timestamp, cutoffDate.Format("2006-01-02 15:04:05"))

	// Fetch old templates
	var templates []models.ImageTemplate
	err = db.Where("created_at < ?", cutoffDate).Find(&templates).Error
	if err != nil {
		log.Fatalf("[%s] ✗ Failed to fetch old templates: %v\n", timestamp, err)
	}

	log.Printf("[%s] Found %d templates older than %d days\n", timestamp, len(templates), days)

	if len(templates) == 0 {
		log.Printf("[%s] ✓ No old templates found. Nothing to clean up.\n", timestamp)
		os.Exit(0)
	}

	log.Printf("[%s] ⚠ Found %d old templates:\n", timestamp, len(templates))
	for _, template := range templates {
		fmt.Println(template.FileKey)
	}

	// Delete old templates from R2 and database atomically
	log.Printf("[%s] Deleting old templates...\n", timestamp)
	deletedFiles := 0
	failedFiles := 0
	deletedRecords := 0
	failedRecords := 0
