...
func main() {
	...
	log.Printf("[%s] Starting subscription amount update for unauthenticated users\n", timestamp)
	...
	query := `
		SELECT DISTINCT
			...
			COUNT(s.id) OVER (PARTITION BY u.id) as subscription_count
		FROM users u
	...
	if len(results) == 0 {
		log.Printf("[%s] ✓ No users found with unauthenticated subscriptions\n", timestamp)
		return
	}

	log.Printf("[%s] Found %d users with unauthenticated subscriptions:\n", timestamp, len(results))
	log.Printf("[%s] ==================================================================================\n", timestamp)
	...
	for i, result := range results {
		...
		log.Printf("[%s]   Subscription Count: %d\n", timestamp, result.SubscriptionCount)
		log.Printf("[%s] ==================================================================================\n", timestamp)
	}
	...
	log.Printf("[%s] ✓ Subscription amount update completed successfully\n", timestamp)
}
