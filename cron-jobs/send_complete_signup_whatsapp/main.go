...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: go run cron-jobs/send_complete_signup_whatsapp/main.go <csv_path>")
	...
	for i, rec := range records {
		...
		log.Printf("scheduled +%s%s at %s UTC", countryCode, rec.phone, scheduledAt.Format("15:04:05"))
	}
...
func loadCSV(path string) (allRows [][]string, records []record, err error) {
	...
	if !okP || !okS {
		return nil, nil, fmt.Errorf("CSV must have 'phone' and 'message_sent' columns (got: %v)", header)
	}
