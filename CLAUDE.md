...
Each cron job is a standalone Go binary in `cron-jobs/<job_name>/main.go`. They are triggered by GitHub Actions workflows (`.github/workflows/`). Jobs use `database.NewCronConnection()`. Key jobs:
...
- `charge_pending_payments`, `retry_failed_billing_cycles`, `process_new_billing_cycles`, `reconcile_payments` — billing automation
- `cleanup_*` — orphan/old file cleanup in R2 and DB
